	...
	protected boolean userAuth() {
		...
		if(tk.equals("")) {
			...
			try {
				...
				ex.printStackTrace();
				mgr.view.error("Could not log in to "+getName()+"; probably caused by bad OAuth request");
				return false;
			}
			...
			try {
				...
				mgr.view.error(ex, "could not open browser to connect");
				return false;
			}
			String req = null;
			try {
			...
			try {
				...
				mgr.view.error("Error while parsing OAuth HTTP request");
				return false;
			}
			...
			try {
				...
				mgr.view.error("Could not save "+getName()+" account to preferences");
			}
		} else {
			accessToken = new Token(tk, tksec);
		}
		
		mgr.view.log("Logged in to "+getName());
		return true;
	}
